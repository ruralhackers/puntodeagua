...
export function DataTable<TData, TValue>({
  ...
  const table = useReactTable({
    ...
    onSortingChange: async (updater) => {
      ...
      if (newSorting.length > 0 && newSorting[0]) {
        await setSortField(newSorting[0].id)
        await setSortDirection(newSorting[0].desc ? 'desc' : 'asc')
      } else {
  ...
  return (
    <div className={hideFooter ? 'space-y-4 w-full' : 'flex flex-1 w-full flex-col space-y-4'}>
      {hideFooter ? (
