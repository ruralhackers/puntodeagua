...
export function NewEngancheCard() {
  const user = useUserStore((state) => state.user)

  if (!user || !canAccessAdminPanel(user.roles)) {
    return null
  }

  return (
    <Link href="/admin/water-points/new" className="group">
      <Card className="h-32 cursor-pointer transition-all duration-200 hover:shadow-md hover:scale-[1.02] bg-gradient-to-br from-teal-50 to-teal-100 border-teal-200 hover:border-teal-300">
        <CardContent className="flex items-center justify-center h-full p-6">
          <div className="flex flex-col items-center gap-3 text-center">
            ...
            <div>
              <CardTitle className="text-teal-800 text-sm font-semibold">Nuevo enganche</CardTitle>
