...
- **Never touch build or dependency infrastructure.** Out of bounds for every task in this plan: `next.config.js`, any `package.json`, `bun.lock`, `apps/webapp/src/server/db.ts`, and anything under `packages/database/`. This plan adds no dependencies and changes no build configuration. If a build or test fails for an infrastructure reason (a missing module, a Prisma/server-only boundary error, a workspace resolution problem), that failure is PRE-EXISTING and not yours: report it in your report and move on. Do not repair it. A subagent that "fixed the build" mid-plan produced exactly this: a rewritten `next.config.js`, `@prisma/client` moved between packages, a rewritten `db.ts`, and a new `server-only` dependency — none of it in scope, all of it reverted.
